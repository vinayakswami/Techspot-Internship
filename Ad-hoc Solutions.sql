...
/* Each driver have differnet ratings in different cities. 'Majid Riordan' have highest 5 rating in 'Alexandria' city.*/  
...
/* 8.Technical manager wants to add an offset value for the application ETA he asked you to for each city the difference between ETA and actual ETA to consider it for his 
experiment (note: if city has no trips it should be also visible). */ 
select id,eta,actual_eta,(eta - actual_eta)as diff_in_ETA from trips ;


/* 9. . Client Engagement Manager noticed that some of the cancelled trips have fake ratings and asked you to replace these ratings to become empty to avoid false scores for 
